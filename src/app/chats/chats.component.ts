...
export class ChatsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!token) {
      // Si no hay token, intenta capturar el `code` desde la URL para intercambiarlo por un token
  ...
  //Solicita el token al backend con el code
  exchangeToken(code: string): void {
    this.messageService.getAccessToken(code).subscribe((response) => {
      // Para cada chat, obtén su último mensaje y la hora
      localStorage.setItem('zohoAccessToken', response.access_token);
      localStorage.setItem('zohoRefreshToken', response.refresh_token);
      // Redirigir a la página de chats después de obtener el token
